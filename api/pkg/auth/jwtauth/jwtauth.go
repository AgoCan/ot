...
const _defaultKey = "gin-admin"

var _defaultOptions = options{
	tokenType:     "Bearer",
	expired:       7200,
	signingMethod: jwt.SigningMethodHS512,
	signingKey:    []byte(_defaultKey),
	keyfunc: func(t *jwt.Token) (interface{}, error) {
		if _, ok := t.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, auth.ErrInvalidToken
		}
		return []byte(_defaultKey), nil
	},
}
...
// GenerateToken 生成令牌
func (j *JWTAuth) GenerateToken(ctx context.Context, userID string) (auth.TokenInfo, error) {
	now := time.Now()
...
func (j *JWTAuth) parseToken(tokenString string) (*jwt.StandardClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &jwt.StandardClaims{}, j.opts.keyfunc)
	if err != nil {
		return nil, err
	} else if !token.Valid {
		return nil, auth.ErrInvalidToken
...
// DestroyToken 销毁令牌
func (j *JWTAuth) DestroyToken(ctx context.Context, tokenString string) error {
	...
	// 如果设定了存储，则将未过期的令牌放入
	return j.callStore(func(store Storer) error {
...
// ParseUserID 解析用户ID
func (j *JWTAuth) ParseUserID(ctx context.Context, tokenString string) (string, error) {
	if tokenString == "" {
	...
	err = j.callStore(func(store Storer) error {
